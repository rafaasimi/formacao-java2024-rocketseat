...
public class AuthCompanyUseCase {
    ...
    public String execute(AuthCompanyDTO authCompanyDTO) throws AuthenticationException {
        var company = companyRepository.findByUsername(authCompanyDTO.getUsername()).orElseThrow(() -> {
            throw new UsernameNotFoundException("Empresa ou senha não encontradas");
        });

        // Verificar senha são iguais
        var passwordMatches = this.passwordEncoder.matches(authCompanyDTO.getPassword(), company.getPassword());

        // Se não for igual, lançar exceção
        if (!passwordMatches) {
        ...
        var token = JWT.create().withIssuer("gestaovagas")
                .withExpiresAt(Instant.now().plus(Duration.ofHours(2))) // Horário de expiração do token (2 horas)
